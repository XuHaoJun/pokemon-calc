...
def create_prompt(question: str) -> str:
  ...
  parts.append('Question:招式劍舞 or 劍舞招式, Translated: {"moves": {"$elemMatch": {"nameDisplay": "劍舞"}}}.')
  parts.append('Question:招式噴射火焰 or 噴射火焰招式, Translated: {"moves": {"$elemMatch": {"nameDisplay": "噴射火焰"}}}.')
  parts.append('Alias:')
  ...
  parts.append('6. Pokemon has resitance(or strengths) to some attack types, is meaning column "typeDefensives" effective < 1 (values: 0, 0.25, 0.5, 1), else if weaknesses is effective > 1 (values: 2, 4), effective == 1 is meaing no strengths or weaknesses.')
  ...
  parts.append('13. Do not generate MongoDB query like: "{"$elemMatch": {"$and": []}}" should use "$all", correct: "{"$elemMatch": {"$all": []}}" incorrect: "{"$elemMatch": {"$and": []}}".')
  parts.append('14. Do not full object match should partial match properties, correct: "{"move.name": "abc"}" incorrect: "{"move": {"name": "abc"}}".')
